...
int *transgraph(int *adjm, int n, int *nout) //every result data is on node 0
{
  ...
  if (me == 0) {
    ...
    for (int i = 1; i < nodes; i++)
      MPI_Recv(num1s + (i - 1) * lenchunk, lenchunk, MPI_INT, i, NEW_MSG, 
    ...
    MPI_Send(num1s + (me - 1) * lenchunk, lenchunk, MPI_INT, 0, NEW_MSG,
             MPI_COMM_WORLD);
  }
  ...
  if (me == 0) {
    ...
    *nout = cumul1s[n];
    ...
    for (int i = 0; i < (*nout); i++) {
      MPI_Recv(out, 1, MPI_2INT, MPI_ANY_SOURCE, NEW_MSG, MPI_COMM_WORLD, 
               &status);
      outm[out[0]] = out[1];
      MPI_Recv(out, 1, MPI_2INT, MPI_ANY_SOURCE, NEW_MSG2, MPI_COMM_WORLD, 
               &status);
      outm[out[0]] = out[1];
    }
  }
